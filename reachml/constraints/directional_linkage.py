...
class DirectionalLinkage(ActionabilityConstraint):
    ...
    """
    def __init__(self, names, parent = None, scales = None, keep_bounds = True, direction = "increase"):
        """
    ...
    def __str__(self):
        clean_names = (pd.Series([self._source]+self._targets).str.replace('_geq_','_$\geq$_').str.replace('_geq_','_$\geq$_').str.replace('_leq_','_$\leq$_').str.split('_').apply(lambda x: ''.join([col[0].upper() + col[1:] for col in x])))
        latex_names = [f"\\textfn{{{n}}}" for n in clean_names]

        s = f"DirectionalLinkage: Actions on {latex_names[0]} will induce to actions on {', '.join(latex_names[1:])}." \
            f"Each unit change in {latex_names[0]} leads to:" + \
            ", ".join([f"{s:1.2f}-unit change in {n}" for n,s in zip(latex_names[1:], self._scales)])
        return s
    ...
    def add_to_gurobi(self, X, A, b):
        ...
        """
        assert(self.keep_bounds == True), "Currently only support changes that keep data point within bounds"

        # simplified constraint
        # a[k] \geq s_k * a[j] for all k in targets
        j = self.indices[0] #j == source index
        target_indices = self.indices[1:]
        ...
        if self._direction == 'increase':
            # a[k] \geq s_k * a[j] for all k in targets
            # => -a[k] + s_k * a[j] \leq 0
            for idx, k in enumerate(target_indices):
                target_direction = self.parent[k].step_direction
                if target_direction != 1:
                    raise ValueError(f"Target feature {k} is not actionable in the decreasing direction")

                x_row = np.zeros(len(self.parent))
                a_row = np.zeros(len(self.parent))
                a_row[k] = -1
                a_row[j] = self.scales[idx]
                X.append(x_row)
                A.append(a_row)
                b.append(0)

        elif self._direction == 'decrease':
            # a[k] \leq s_k * a[j] for all k in targets
            # => a[k] - s_k * a[j] \leq 0
            for idx, k in enumerate(target_indices):
                target_direction = self.parent[k].step_direction
                if target_direction != -1:
