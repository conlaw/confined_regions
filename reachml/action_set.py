...
class ActionSet(object):
    ...
    def __init__(self, X, names=None, **kwargs):
        ...
        self._population_elements = {
            n: ActionElement.from_values(name=n, values=X[:, j])
            for j, n in enumerate(self._names)
        }
        self._constraints = _ConstraintInterface(parent=self)
        self._population_constraints = _ConstraintInterface(parent=self)
    ...
    @property
    def population_constraints(self):
        return self._population_constraints
    
    def set_population_bounds(self, lb, ub):
        """
        Set bounds for the population elements
        :param lb: lower bound
        :param ub: upper bound
        """
        assert len(lb) == len(self._population_elements)
        assert len(ub) == len(self._population_elements)

        for j, a in enumerate(self._population_elements.values()):
            a.lb = lb[j]
            a.ub = ub[j]
    ...
    def get_variable_types(self):
        """
        :return: list of variable types for each feature
        """
        return [a.variable_type for a in self]

    def get_data_bounds(self, bound_type, part=None):
        """
        Returns upper and lower bounds for the data features
        ...
        if part is None:
            out = [
                aj.get_data_bound(bound_type=bound_type)
                for aj in self
            ]
        else:
            out = [
                aj.get_data_bound(bound_type=bound_type) if j in part else 0.0
                for j, aj in enumerate(self)
            ]
        return out
    
    def get_population_bounds(self, bound_type, part=None):
        assert bound_type in ("lb", "ub"), f"invalid bound_type: {bound_type}"
        if part is None:
            out = [
                aj.get_data_bound(bound_type=bound_type)
                for aj in self._population_elements.values()
            ]
        else:
            out = [
                aj.get_data_bound(bound_type=bound_type) if j in part else 0.0
                for j, aj in enumerate(self._population_elements.values())
            ]
        return out
    ...
    def get_bounds(self, x, bound_type, part=None):
        ...
        if part is None:
            if x is None:
                out = [
                    aj.get_action_bound(None, bound_type=bound_type)
                    for aj in self
                ]
            else:
                out = [
            ...
            if x is None:
                out = [
                    aj.get_action_bound(None, bound_type=bound_type) if j in part else 0.0
                    for j, aj in enumerate(self)
                ]
            else:
                out = [
    ...
    def to_latex(self):
        ...
        """
        names = [aj.name for aj in self]
        clean_names = (pd.Series(names).str.replace('_geq_','_$\geq$_').str.replace('Geq','_$\geq$_').str.replace('_leq_','_$\leq$_').str.split('_').apply(lambda x: ''.join([col[0].upper() + col[1:] for col in x])))

        df = pd.DataFrame({
            "name": ['\\textfn{'+aj+'}' for aj in clean_names],
            "ub": [aj.ub for aj in self],
            "lb": [aj.lb for aj in self],
            "actionable": [aj.actionable for aj in self],
            "step_direction": [aj.step_direction for aj in self],
            "variable_type": [aj.variable_type for aj in self]
        })
        tex_binary_str = "$\\{0,1\\}$"
        ...
        df.loc[up_idx, "mutability"] = "+"
        df.loc[dn_idx, "mutability"] = "-"
        df["actionable"] = df["actionable"].map(
            {False: "No", True: "Yes"}
        )  # todo change
        df = df.drop(["step_direction"], axis=1)
        df = df[["name", "variable_type", "lb", "ub", "actionable", "mutability"]]
        clean_columns = {
                "name": "Name",
                "variable_type": "Type",
                "lb": "LB",
        ...
        for k, v in clean_columns.items():
            clean_columns[k] = '\\textheader{'+clean_columns[k]+'}'
        df = df.rename(
    ...
    def population_to_latex(self):
        """
        :param action_set: ActionSet object
        :return: formatted latex table summarizing the action set for publications
        """
        names = [aj.name for aj in self._population_elements.values()]
        clean_names = (pd.Series(names).str.replace('_geq_','_$\geq$_').replace('_leq_','_$\leq$_').str.split('_').apply(lambda x: ''.join([col[0].upper() + col[1:] for col in x])))

        df = pd.DataFrame({
            "name": ['\\textfn{'+aj+'}' for aj in clean_names],
            "ub": [aj.ub for aj in self._population_elements.values()],
            "lb": [aj.lb for aj in self._population_elements.values()],
            "variable_type": [aj.variable_type for aj in self._population_elements.values()]
        })
        tex_binary_str = "$\\{0,1\\}$"
        tex_integer_str = "$\\mathbb{Z}$"
        tex_real_str = "$\\mathbb{R}$"
        ...
        df = df[["name", "variable_type", "lb", "ub"]]
        clean_columns = {
                "name": "Name",
                "variable_type": "Type",
                "lb": "LB",
