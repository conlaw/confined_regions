...
class ActionElement:
    ...
    def get_action_bound(self, x, bound_type):
        ...
        #Handle case where x is None can take any range of value between [lb, ub]
        #if looking for ub then set x=lb, else set x=ub
        if x is None:
            x = self.lb if bound_type == "ub" else self.ub
    ...
    def get_data_bound(self, bound_type):
        assert bound_type in ("lb", "ub")
        if bound_type == "ub":
            return self.ub
        else:
