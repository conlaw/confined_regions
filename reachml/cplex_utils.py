...
# variable stamping
VTYPE_TO_CPXTYPE = {int: "I", bool: "I", float: "I"}
RELAX_VTYPE_TO_CPXTYPE = {int: "C", bool: "C", float: "C"}
...
def get_cpx_variable_types(action_set, indices=None, relax=False):
    if indices is None:
        indices = range(len(action_set))
    vtype_dict = VTYPE_TO_CPXTYPE if not relax else RELAX_VTYPE_TO_CPXTYPE
    out = "".join(
        [
            vtype_dict[vt]
...
def get_cpx_variable_args(name, obj, ub, lb, vtype, relax=False):
    """
