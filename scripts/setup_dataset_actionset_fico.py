...
def process_dataset(raw_df):
    ...
    #df["YearsOfAccountHistory_geq_3"] = YearsOfAccountHistory >= 3
    #df["YearsOfAccountHistory_geq_5"] = YearsOfAccountHistory >= 5
    #df["YearsOfAccountHistory_geq_7"] = YearsOfAccountHistory >= 7
...
def simple_1D(data):
    ...
    immutable_features += [
        ...
        #
        #"YearsOfAccountHistory_geq_3",
        #"YearsOfAccountHistory_geq_5",
        #"YearsOfAccountHistory_geq_7",
        #
        "AnyTrade120DaysDelq",
        "AnyTrade90DaysDelq",
        "AnyTrade60DaysDelq",
...
def complex_1D(data):
    ...
    for n in A.name:
        ...
        #if any(col in n for col in increasing_cols):
        #    A[n].step_direction = 1
...
def complex_nD(data):
    ...
    A.population_constraints.add(
                constraint=ThermometerEncoding(
                        names=[n for n in A.name if "ExternalRiskEstimate" in n],
                        step_direction=1,
                        )
                )
    A.constraints.add(
    ...
    A.constraints.add(
            constraint=ThermometerEncoding(
                    names=['MostRecentTradeWithinLast2Years', "MostRecentTradeWithinLastYear"],
                    step_direction=0,
                    )
            )
...
def generate_possible_values_dictionary(action_set):
    possible_values_df = {}
    col = 'ExternalRiskEstimate_geq_'
    ...
    possible_values_df['YearsOfAccountHistory'] = pd.DataFrame(num_range(0, 50), columns = ['YearsOfAccountHistory'])

    for col in ['AvgYearsInFile_geq_']:
        col_names = [a.name for a in action_set if col in a.name]
        possible_values_df[col] = pd.DataFrame(therm_lower(len(col_names)), columns = col_names)

    for col in ['MostRecentTradeWithin']:
        col_names = [a.name for a in action_set if col in a.name]
        possible_values_df[col] = pd.DataFrame(therm_upper(len(col_names)), columns = col_names)

    bin_cols = ['AnyDerogatoryComment', 'AnyTrade120DaysDelq', 'AnyTrade90DaysDelq', 'AnyTrade60DaysDelq', 'AnyTrade30DaysDelq', 'NoDelqEver']
    for col in bin_cols:
        possible_values_df[col] = pd.DataFrame(num_range(0,1), columns = [col])

    for col in ['YearsSinceLastDelqTrade_leq_', 'NumInstallTrades_geq_', 'NumInstallTradesWBalance_geq_', 'NumRevolvingTrades_geq_', 'NumRevolvingTradesWBalance_geq_',
                'NetFractionInstallBurden_geq_', 'NetFractionRevolvingBurden_geq_']:
        col_names = [a.name for a in action_set if col in a.name]
        possible_values_df[col] = pd.DataFrame(therm_lower(len(col_names)), columns = col_names)

    possible_values_df['NumBank2NatlTradesWHighUtilizationGeq2'] = pd.DataFrame(num_range(0,1), columns = ['NumBank2NatlTradesWHighUtilizationGeq2'])
...
for name in settings["action_set_names"]:
    ...
    if settings['generate_regions']:
        A = ActionSet(data.X_df)
        immutable_features = [a.name for a in A if "ExternalRiskEstimate" in a.name]  + ['YearsOfAccountHistory']
        immutable_features += [
                "NoDelqEver",
                "AnyDerogatoryComment",
                #
        ...
        regions = data.X_df[immutable_features].drop_duplicates().to_dict(orient='records')
        fileutils.save(
                    regions,
    ...
    if settings['generate_generator_dict']:
        possible_value_dict = generate_possible_values_dictionary(A)
        fileutils.save(
                    possible_value_dict,
