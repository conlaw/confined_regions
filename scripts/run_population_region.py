...
settings = {
    ...
    "model_type": "logreg",
    "method_name": "region_verification",
    "total_regions": 200,
    "random_seed": 2338,
}
...
region = fileutils.load(get_region_file(**settings))
...
results = []
for idx, region in tqdm(enumerate(region), total=len(region)):
    action_set = fileutils.load(get_action_set_file(**settings))
    ...
    try:
        ...
        if not flag:
            ...
            with open(os.devnull, "w") as devnull:
                with contextlib.redirect_stdout(devnull):
                    flag, output = pop_verifier.verify()
            exec_time = time.process_time() - start_time
            if np.all(output['u'] == action_set.get_population_bounds('ub')) and np.all(output['l'] == action_set.get_population_bounds('lb')):
                certify_robustness = True
            else:
            ...
            results.append(
                                {   "region_idx": idx,
                                    "a": output,
                                    "found_x": True,
                                    "certifies_recourse": False,
                                    "certifies_robustness": certify_robustness,
                                    "exec_time": exec_time,
                                }
                            )
            ...
            results.append(
                                {   "region_idx": idx,
                                    "a": None,
                                    "found_x": False,
                                    "certifies_recourse": True,
                                    "certifies_robustness": False,
                                    "exec_time": exec_time,
                                }
                            )

    except Exception:
        print('Region index: ', idx)
