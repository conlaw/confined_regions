...
from utils import check_processing_loss, check_responsiveness, tabulate_actions, tally
import pprint
...
def process_dataset(raw_df):
    ...
    #df["CreditLineUtilization"] = round(utilization*10)
    utilization_thresholds = (0.1, 0.2, 0.5, 0.7, 1.0)  # old versions: (3, 7, 11)
    for t in utilization_thresholds:
        df[f"CreditLineUtilization_geq_{int(t*100)}"] = utilization >= t
...
def generate_possible_values_dictionary():
    possible_values_df = {}
    possible_values_df['Age'] = pd.DataFrame(num_range(21, 103), columns = ['Age'])
    possible_values_df['NumberOfDependents'] = pd.DataFrame(num_range(0, 20), columns = ['NumberOfDependents'])
    possible_values_df['DebtRatio_geq_1'] = pd.DataFrame(num_range(0, 1), columns = ['DebtRatio_geq_1'])

    therm_cols = ['MonthlyIncome_geq_', 'CreditLineUtilization_geq_', 'RealEstateLoans', 'CreditLinesAndLoans', 'user_retweeted_geq_']
    ...
    possible_values_df['HistoryOfLatePayment'] = pd.DataFrame(num_range(0, 1), columns = ['HistoryOfLatePayment'])
    possible_values_df['HistoryOfDelinquency'] = pd.DataFrame(num_range(0, 1), columns = ['HistoryOfDelinquency'])
