...
def get_region_file(data_name, action_set_name, **kwargs):
    """
    :param data_name: string containing name of the dataset
    :return: file name
    """
    assert isinstance(data_name, str) and len(data_name) > 0
    assert isinstance(action_set_name, str) and len(action_set_name) > 0
    f = data_dir / f"{data_name}_{action_set_name}.region"
    return f

def get_generator_file(data_name, action_set_name, **kwargs):
    """
    :param data_name: string containing name of the dataset
    :return: file name
    """
    assert isinstance(data_name, str) and len(data_name) > 0
    assert isinstance(action_set_name, str) and len(action_set_name) > 0
    f = data_dir / f"{data_name}_{action_set_name}.generator"
    return f
...
def get_benchmark_results_csv_file(
    data_name, action_set_name, method_name, model_type, **kwargs
):
    assert isinstance(data_name, str) and len(data_name) > 0
    assert isinstance(method_name, str) and len(action_set_name) > 0
    ...
    f = (
        results_dir
        / f"{data_name}_{action_set_name}_{model_type}_{method_name}.csv"
    )
    return f
